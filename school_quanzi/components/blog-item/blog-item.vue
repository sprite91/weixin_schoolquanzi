<template>
	<view class="blogitem">
		...
		<view class="info">
			...
			<text>{{item.view_count ? item.view_count : "浏览量"}}</text></view>
			<view class="box" @click="goDetail"><text class="iconfont icon-a-5-xinxi"></text> 
			<text>{{item.comment_count && item.comment_count>0 ? item.comment_count : "评论"}}</text></view>
			<view class="box" :class="item.isLike ? 'active':''" @click="clickLike"><text class="iconfont icon-a-106-xihuan"></text>
			<text>{{item.like_count && item.like_count>0 ? item.like_count : "点赞"}}</text></view>
		</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			goDetail(){
				uni.navigateTo({
					url:"/pages_blog/detail/detail?id="+this.item._id
				})
			},
			//单击图片
			clickPic(index){
